...
https://github.com/microsoft/CameraTraps/blob/main/detection/tf_detector.py
...
from api_utils import truncate_float
from config import CONF_DIGITS, COORD_DIGITS, FAILURE_INFER

# Setup logger & environment variables
logger = logging.getLogger(__name__)

logger.info("TensorFlow version:", tf.__version__)
logger.info("Is GPU available? tf.test.is_gpu_available:", tf.test.is_gpu_available())
...
class TFDetector:
    ...
    def generate_detections_one_image(self, image, image_id, detection_threshold):
        ...
        try:
            ...
            for b, s, c in zip(boxes, scores, classes):
                if s > detection_threshold:
                    detection_entry = {
                        ...
                        "conf": truncate_float(float(s), precision=CONF_DIGITS),  # cast to float for json serialization
            ...
            result["max_detection_conf"] = truncate_float(float(max_detection_conf), precision=CONF_DIGITS)
            result["detections"] = detections_cur_image

        except Exception as e:
            result["failure"] = FAILURE_INFER
            logger.error("TFDetector: image {} failed during inference: {}".format(image_id, str(e)))
